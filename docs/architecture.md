...
### expect/actual Path (perTargetGeneration)

When per-target generation is enabled, use the generated extension function directly from `commonMain`:

```kotlin
val api = jvmKtorfit.createExampleApi()
```

The `expect fun Ktorfit.createExampleApi()` declared by the user in `commonMain` is resolved by the compiler to the `actual` generated by KSP in each platform target. No compiler plugin, no registry — just standard Kotlin multiplatform `expect`/`actual`. This is the same pattern used by Room for Kotlin Multiplatform.
