...
fun createFileSpec(
    ...
    if (createRegistration) {
        builder.addImport("de.jensklingenberg.ktorfit.optins", "KtorfitFactoryRegistry")
        builder.addProperty(registrationPropertySpec)
    }
 ...
 * `Ktorfit.createUsingRegistry<T>()` can resolve the API without a separate
 * registry file — avoiding file-name collisions in multi-module projects.
 *
 * Generates:
 *   @Suppress("unused")
 *   private val __ktorfit_registration: Unit =
 *       run { KtorfitFactoryRegistry.register(TestApi::class, _TestApiProvider()) }
 */
private fun createRegistrationPropertySpec(classData: ClassData): PropertySpec =
    PropertySpec
        ...
        .addAnnotation(
            ...
            "run { KtorfitFactoryRegistry.register(%L::class, ${classData.providerName}()) }",
            classData.name,
        ).build()
